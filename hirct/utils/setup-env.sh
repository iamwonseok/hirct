...
elif [[ -d "${PROJECT_ROOT}/third_party/circt/build" ]]; then
  export CIRCT_BUILD="${PROJECT_ROOT}/third_party/circt/build"
  log_info "Auto-detected CIRCT_BUILD (submodule): ${CIRCT_BUILD}"
...
if [[ -d "${VCS_HOME}" ]]; then
  ...
  # kernel 6.x (Ubuntu 24.04+)에서 VCS가 libelf.so.1을 찾으려면 시스템 lib 경로 필요
...
# All packages (including optional) are now in requirements.txt
